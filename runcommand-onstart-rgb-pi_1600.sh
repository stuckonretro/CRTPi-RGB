...
# change timings for 320.txt to 1600x240p
...
# change timings for 320x224 & 384x224 systems to 1600x224p
	elif 
		[[ "$system" == "megadrive" ]] || 
		[[ "$system" == "segacd" ]] || 
		[[ "$system" == "sega32x" ]] || 
		[[ "$system" == "fba" ]] || 
		[[ "$system" == "neogeo" ]] ; then
			vcgencmd hdmi_timings 1600 1 73 157 204 224 1 12 3 23 0 0 0 60 0 32000000 1 > /dev/null #CRTPi 1600x224p Timing Adjusted
			tvservice -c "DMT 87" > /dev/null
			fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 1600 -yres 224 > /dev/null #24b depth
			
# change timings for 320x200 systems to 1600x200p
	elif 
		[[ "$system" == "quake" ]] || 
		[[ "$system" == "doom" ]] ; then
			vcgencmd hdmi_timings 1600 1 73 157 204 200 1 24 3 35 0 0 0 60 0 32000000 1 > /dev/null #CRTPi 1600x200p Timing Adjusted
			tvservice -c "DMT 87" > /dev/null
			fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 1600 -yres 200 > /dev/null #24b depth
			
# change timings for 320x192 systems to 1600x192p
	elif 
		[[ "$system" == "atari2600" ]] || 
		[[ "$system" == "atari5200" ]] || 
		[[ "$system" == "atari800" ]] || 
		...
		[[ "$system" == "zxspectrum" ]] ; then
			vcgencmd hdmi_timings 1600 1 73 157 204 192 1 28 3 39 0 0 0 60 0 32000000 1 > /dev/null #CRTPi 1600x192p Timing Adjusted
			tvservice -c "DMT 87" > /dev/null
			fbset -depth 8 && fbset -depth 16 && fbset -depth 24 -xres 1600 -yres 192 > /dev/null #24b depth
			
# change timings for 320x240 systems to 1600x240p
	elif 
		[[ "$system" == "psx" ]] || 
		[[ "$system" == "dreamcast" ]] || 
		[[ "$system" == "saturn" ]] || 
